...
- **CTA trigger levels** — a rough "where do trend-followers flip" gauge on the positioning panel: SPY's distance to its 50/100/200-day SMAs (the MA-based levels CTAs key off), with a state (trend-long / de-grossing / short) and the nearest MA as the first de-gross/re-gross trigger. A free MA proxy for CTA positioning, not the proprietary model. Part of `GET /market/positioning` (`cta`)
...
- **Theme rotation** — the cross-cutting themes GICS sectors miss: curated baskets for **Cybersecurity, Agentic AI / Software, AI Infrastructure / Semis, Robotics, Quantum, Space & Defense, Power / Grid / Nuclear, Biotech / GLP-1** (seeded from the canonical thematic indices — CIBR, IGV, SMH, BOTZ, QTUM, UFO, GRID, XBI — de-duped to liquid US names). Each scored by **RS vs SPY + RS acceleration** into RRG quadrants (Leading / Improving / Weakening / Lagging) using constituent **medians**, and each expandable to its **top-RS names to focus on** (click → Analyze). Answers "when is a theme rotating" + "what to buy in it". Cached 6h. `GET /market/themes`
...
- **Market-implied Quad (regime capstone)** — inspired by Hedgeye's GIP framework: infers the Growth/Inflation regime the *market is trading* (Quad 1 Goldilocks / 2 Reflation / 3 Stagflation / 4 Deflation) from sector leadership + style factors + credit + breadth, with the matching sector/factor/asset **playbook** and a transparent list of which signals voted. **Dual-horizon** (Hedgeye's own split): **Quarterly = climate** (longer-term, ~3-month lookbacks + 200-DMA breadth) and **Monthly = weather** (intermediate, ~1-month lookbacks + 50-DMA breadth), with an aligned/transition flag. A heuristic market-implied read, **not** a GDP/CPI nowcast. Quad 1/2 = green light for momentum; 3/4 = defend. Cached 6h. `GET /market/regime`
- **Market Gamma (index dealer-gamma regime)** — a compact tile on the Macro page that auto-runs **SPY + QQQ GEX** (from the CBOE chain) so you don't have to analyze them by hand: the gamma sign (**positive = calm/pinning, negative = amplifying**), the **zero-gamma flip** (above = stabilizing, below = accelerant), and the call/put gamma walls, with a plain-English read. Tells you whether index breakouts are likely to run (negative gamma) or fade (positive). Cached 15min. `GET /market/gamma`
- **Fundamental Quad (the data)** — the macro **data** read that complements the price-implied one: Hedgeye-style GIP rate-of-change from free FRED series — **Real GDP YoY** (GDPC1, climate/quarterly), **Industrial Production YoY** (INDPRO, weather/monthly), both vs **Headline CPI YoY** (CPIAUCSL). The panel shows a **Tape vs Data** verdict: when the price-implied Quad and the fundamental Quad **diverge**, the market is pricing one regime while the data turns toward another (Hedgeye's "bullish until it isn't"). Trailing (latest released data), not a forward nowcast. Cached 12h. `GET /market/gip`

### Frontend
- **Analyze page** — Deep-dive for any individual stock: a **candlestick price chart** (TradingView lightweight-charts, free/no-key) with EMA21/50 and the active setup's **entry/stop/targets + Risk Range band drawn on it**, all 7 setups, RSI/MACD/ADX, MA stack, Weinstein stage, ICS, A/D Net, 10-item signal checklist (incl. Minervini Trend Template), Minervini Trend Template criteria card, a **"Why this score" per-driver P Score breakdown** (plain-English bullets — what's pushing the grade up/down, conviction, regime, penalty), a **Regime Fit** card (macro tailwind/headwind), early warnings, **multi-timeframe alignment** (daily + weekly + monthly, no extra API calls), and a **Fibonacci grid** (retracement ladder + golden pocket + extension targets, anchored to the dominant ~6-month swing)
...
- **Regime Fit (macro → stock bridge)** — Brings the macro layer down to a single stock: does this setup have the macro wind at its back? A −3…+3 score from three signals — the stock's **sector leadership** (ticker → GICS sector → SPDR ETF → rotation quadrant), the live **Quad posture** (weather Quad 1/2 = green light, 3/4 = defend), and the **momentum factor** regime — rendered as a **tailwind / neutral / headwind** verdict. Shows on the Analyze page (`regime_fit`) and as a **Regime column on the scanner table**, with a **"Tailwind only" filter** to narrow the list to setups the regime supports. Sector mapping covers the S&P 500; other names score on Quad + momentum only. Context for conviction/sizing, **not a P Score input**
- **Risk Range + TRADE/TREND/TAIL (Hedgeye-style)** — Per-symbol card on the Analyze page, computed purely from price/volatility. (1) **Risk Range** — a volatility-bounded immediate-term probable band (centre = EMA20, half-width = realised daily vol × √horizon); momentum names ride the top, the low end is the lower-risk pullback entry. Shows where price sits in the band. (2) **TRADE (~3wk, SMA21) / TREND (~3mo, SMA63) / TAIL (~1yr, SMA200)** — three trend durations, each with the explicit level you're bullish above / bearish below, plus a combined state (bullish_all / bullish_trade_trend / mixed / rolling_over / bearish_all). The actionable *line* a plain MA-stack lacks. Context for entries/stops, not a P Score input. In the `/analyze` payload as `risk_range`
- **Short-volume pressure (leading)** — Per-symbol card on the Analyze page (lazy-loaded). FINRA's consolidated **daily Reg SHO short-sale-volume** file (free, no key, Render-friendly) over ~10 trading days: short-volume % level, trend, and a directional read against recent price (elevated shorts into a *rising* stock = squeeze fuel; into a *falling* one = bearish confirmation). This is short-sale **volume** (a daily flow proxy), **not** bi-monthly short interest / days-to-cover — read the deviation from the ~45-50% baseline. Confluence layer, not a P Score input. `GET /short-volume/{symbol}`
- **Insider activity (leading)** — Per-symbol card on the Analyze page (lazy-loaded). **SEC EDGAR Form 4** filings (free, no key) over ~120 days: open-market **purchases (code P)** vs **sales (code S)**, with cluster-buy detection (≥2 distinct insider buyers) and the largest recent buys (name, title, size, date). Buys are the leading signal and weighted heavily; selling is down-weighted as noise; grants/exercises/tax/gifts excluded. Confluence layer, not a P Score input. `GET /insider/{symbol}`
...
- **Dashboard** — the scanner home: a compact **Macro snapshot** strip (dual-horizon Quad at a glance, links to the Macro page), scan controls, advanced filter panel, stats bar (Total / Grade A / EPs), full results table
- **Macro page** (`/macro`) — the dedicated market-context section (the platform has grown beyond per-stock analysis): the dual-horizon **Market-Implied Quad** capstone, **Market Positioning** dial, **Market Breadth**, **Sector Rotation** (with multi-horizon heatmap), and **Style-Factor Leadership** — top-down regime context for sizing and where to hunt, none of which feed the per-stock P Score
...
| `GET` | `/market/themes` | Theme rotation: 8 curated thematic baskets scored by RS vs SPY + acceleration (RRG quadrants) + top-RS leaders per theme. Cached 6h |
| `GET` | `/market/regime` | Market-implied Quad: Growth/Inflation regime (Goldilocks/Reflation/Stagflation/Deflation) from sectors+factors+credit+breadth, dual-horizon, with playbook. Cached 6h |
| `GET` | `/market/gip` | Fundamental Quad: FRED GDP/CPI/IP rate-of-change (the data read), dual-horizon, for tape-vs-data divergence. Cached 12h |
| `GET` | `/market/gamma` | Index dealer-gamma regime: SPY + QQQ GEX (sign, zero-gamma flip, call/put walls). Cached 15min |
| `GET` | `/bars/{symbol}` | Recent daily OHLCV (lightweight-charts format) for the Analyze price chart. `?days=` (30–500) |
...
│   │   ├── themes.py            Theme rotation: 8 curated thematic baskets, RRG quadrants + top-RS leaders (leading)
│   │   ├── factors.py           Style-factor leadership: Momentum/Beta/Vol/Short-Interest High−Low spreads (leading)
│   │   ├── regime.py            Market-implied Quad: Growth/Inflation regime + playbook (capstone, the tape)
│   │   ├── macro_quad.py        Fundamental Quad: FRED GDP/CPI/IP rate-of-change (the data, tape-vs-data)
│   │   ├── gamma.py             Market Gamma: index (SPY/QQQ) dealer-gamma regime + flip + walls
│   │   ├── regime_fit.py        Regime Fit: per-stock macro tailwind/headwind (sector+Quad+factor)
