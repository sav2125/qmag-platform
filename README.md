...
- **Options (leading)** — Per-symbol forward-looking layer from the near-dated chain (≤45d): **ATM implied volatility**, **IV Rank** (IBKR-style IV ÷ 30-day realised vol → options rich/cheap), the options-implied **expected move** (± % / ± $ by expiry, for sizing catalyst targets/stops), **skew** (OTM put IV − call IV → hedging vs demand), put/call **volume** & **open-interest** ratios, an **unusual-activity** flag (volume ÷ OI), **max pain** (OI-weighted pin price + distance from spot), the **ACI level** (delta-adjusted-OI sentiment — Black-Scholes-Δ-weighted bullish vs bearish OI, from each snapshot), and **OI-derived support/resistance** (put walls below / call walls above). Distilled into a sentiment lean + plain-English read. Options price *expectations*, so they genuinely lead — but it's a **context/confluence layer, not a P Score input**. **Dual data source:** yfinance chains primary (full card incl. OI metrics); on cloud IPs where yfinance is blocked (Render), it falls back to **Alpaca** options snapshots and inverts IV via Black-Scholes from quotes — Alpaca's free feed has no open interest, so the OI-based metrics (P/C OI, max pain, ACI, OI walls) are unavailable on the fallback while IV / IV Rank / expected move / skew / P-C volume still populate. The `source` field shows which was used
